...
func NewClient(cfg *config.Config, dataQueue *queue.Queue, monitor *monitor.Monitor, analyzer *analyzer.Analyzer, broadcaster broadcast.Broadcaster) (*Client, error) {
	...
	// 订阅主题
	// 连接丢失处理函数
	opts.SetOnConnectHandler(func(mqttClient mqtt.Client) { // 当MQTT broker连接成功时触发
...
func (c *Client) subscribeTopics(mqttClient mqtt.Client) error {
	...
	// 订阅所有主题
	// 订阅主题，设置QoS为1，回调messageHandler处理消息
	for _, topic := range topics {
		token := mqttClient.Subscribe(topic, 1, c.messageHandler()) // 订阅主题，设置QoS为1，回调messageHandler处理消息
		token.Wait()
		if token.Error() != nil {
...
func (c *Client) processBatchMessages() {
	...
	// 清空批量消息
	c.batchMessages = make([]map[string]any, 0)
